...
import httpx
from trading_service.models.models import Transaction, Asset, TransactionType
from trading_service.schemas.schemas import TransactionCreate
from trading_service.exceptions.handlers import NotFoundError, BusinessLogicError

import os
PORTFOLIO_SERVICE_URL = os.getenv("PORTFOLIO_SERVICE_URL", "http://localhost:8001") + "/api/v1/portfolios"
...
def get_all(
    ...
    if portfolio_id:
        try:
            resp = httpx.get(f"{PORTFOLIO_SERVICE_URL}/{portfolio_id}", timeout=2.0)
            if resp.status_code != 200:
                raise NotFoundError("Портфель (remote)", portfolio_id)
        except httpx.RequestError:
            raise BusinessLogicError("Portfolio Service недоступний")
...
def create(db: Session, data: TransactionCreate) -> Transaction:
    """
    Купівля або продаж активу. Мікросервісна валідація портфеля.
    """
    try:
        resp = httpx.get(f"{PORTFOLIO_SERVICE_URL}/{data.portfolio_id}", timeout=2.0)
        if resp.status_code != 200:
            raise NotFoundError("Портфель (remote)", data.portfolio_id)
    except httpx.RequestError:
        raise BusinessLogicError("Portfolio Service недоступний")
